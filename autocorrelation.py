...
for yr in ['2010', '2011', '2012', '2013', '2014', '2015', '2016']:#input('What year? (xxxx) ')
    print(yr)
...
decorrelation = np.where(1/exp(1) < mean_correlation[len(mean_correlation)//2:])
print(len(mean_correlation)//2)
print(decorrelation)

print('Decorrelation at: ', decorrelation * np.mean(distance)/1000, ' km')
pl.xlim([-150, 150])
pl.xlabel('lag (spacing = ' + str(np.around(np.mean(distance)/1000, decimals=3)) + ' km)')
pl.ylabel('Correlation Coefficient')
pl.savefig('/Users/jmh2g09/Documents/PhD/Data/Quality Control/autocorr.png', format='png')
pl.close()