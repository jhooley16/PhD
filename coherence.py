...
pl.xlabel('Month')
pl.savefig('/Users/jmh2g09/Documents/PhD/Data/Coherence/ice_timeseries.png', dpi=300, transparent=True, bbx_inches='tight')
pl.close()
...
pl.ylabel('SSH anomaly (m)')
pl.savefig('/Users/jmh2g09/Documents/PhD/Data/Coherence/timeseries.png', dpi=300, transparent=True, bbx_inches='tight')
pl.close()

for num in range(len(f)):
    if np.isfinite(1/f[num]):
        pl.figure()
        pl.clf()
        ...
        m.pcolor(stereo_x, stereo_y, np.ma.masked_invalid(coherr[:, :, num]))
        m.colorbar()
        pl.clim([0, 1])
        ...
        m.drawmeridians(np.arange(-180., 181., 20.), labels=[0, 0, 0, 1])
        stereo_x, stereo_y = m(grid_lon, grid_lat)
        m.pcolor(stereo_x, stereo_y, np.ma.masked_invalid(coherr_2[:, :, num]))
        m.colorbar()
        pl.clim([0, 1])
        m.scatter(location_x[0], location_y[0], marker='$1$', s=60, color='k', zorder=100)
        m.scatter(location_x[1], location_y[1], marker='$2$', s=60, color='k', zorder=100)
