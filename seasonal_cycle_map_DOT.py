...
dot_anom_ts = np.zeros((59, 361, len(time)))
...
for ilat in range(len(lat)):
    for ilon in range(len(lon)):
        ...
        if np.any(np.isfinite(dot_anom_ts[ilat, ilon, :])):
            regress = stats.linregress(time, funct.inpaint_nans(dot_anom_ts[ilat, ilon, :]))
...
m.pcolor(stereo_x, stereo_y, np.transpose(np.ma.masked_invalid(dot_anom_regress)), cmap='RdBu_r')
m.colorbar()
pl.clim(20, -20)
pl.title(mode)
pl.savefig('/Users/jmh2g09/Documents/PhD/Data/Seasonal/' + mode + '_DOT_trend_map.png', transparent=True, dpi=300)
pl.close()